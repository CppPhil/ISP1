...
{
    std::vector<typename graph_undirected<NodeIdentifier, int, Cost>::node::edge> edges{
        graph.get_edges(nodeToExpand)
    };

    std::vector<IdentifierWithCost<NodeIdentifier>> result{};

    for (const typename graph_undirected<NodeIdentifier, int, Cost>::node::edge& edge : edges) {
        typename graph_undirected<NodeIdentifier, int, Cost>::const_iterator target{edge.target()};
...
{
    typename std::vector<Path<NodeIdentifier>>::iterator insertionPoint{pl::algo::lower_bound(openList, 
        pathToInsert,
        [&heuristic](const Path<NodeIdentifier>& lhs, const Path<NodeIdentifier>& rhs) {
            return 
                (lhs.g() 
                + heuristic(lhs.back().nodeIdentifier))
    ...
    Heuristic heuristic)
{
    ...
    while (not openList.empty()) {
        ...
        if (pl::algo::find(closedList, p.back().nodeIdentifier) == std::end(closedList)) {
            ...
            const std::vector<SearchPath> newPaths{generateNewPaths(p, children)};
