...
int main()
{
    ...
    auto heuristic
        = [&straight_line_Bucharest,
           TARGET](const graph_undirected<string, int, int>::const_iterator& it)
        -> double { return straight_line_Bucharest[it->first]; };
    Map::search_path pastar{map.astar(START, TARGET, heuristic)};

    Map::search_path pdijkstra{map.dijkstra(START).get_path(TARGET)};

    const size_t nr_digits_iddfs{
        static_cast<size_t>(piddfs.empty() ? 0 : log10(piddfs.size()) + 1)};
    cout << "\nCalculation of the shortest path from '" << START
         << "' to 'Bucharest' with several search algorithms:\n"
         << "              | \x1B[34m\x1B[1mBreadth-First Search\x1B[0m |  "
            "\x1B[34m\x1B[1mDepth-First Search\x1B[0m  | "
            "\x1B[34m\x1B[1mDepth-Limited Search (5)\x1B[0m |  "
            "\x1B[34m\x1B[1mUniform-Cost Search\x1B[0m | "
            "\x1B[34m\x1B[1mIterative-Deepening Depth-First Search ("
         << piddfs.size()
         << ")\x1B[0m |       \x1B[34m\x1B[1mA* Search\x1B[0m      |        "
            "\x1B[34m\x1B[1mDijkstra\x1B[0m      |\n"
         << "--------------|----------------------|----------------------|-----"
            "---------------------|----------------------|---------------------"
            "----------------------"
         << string(nr_digits_iddfs, '-')
         << "|----------------------|----------------------|\n"
         << "  Total cost  |\x1B[31m" << setw(13) << pbfs.total_cost()
         << "         \x1B[0m|\x1B[31m" << setw(14) << pdfs.total_cost()
         << "        \x1B[0m|\x1B[31m" << setw(15) << pdls.total_cost()
         << "           \x1B[0m|\x1B[31m" << setw(14) << pucs.total_cost()
         << "        \x1B[0m|\x1B[31m" << setw(25) << piddfs.total_cost()
         << string(nr_digits_iddfs, ' ') << "                  \x1B[0m|\x1B[31m"
         << setw(14) << pastar.total_cost() << "        \x1B[0m|\x1B[31m"
         << setw(14) << pdijkstra.total_cost() << "        \x1B[0m|\n"
         << "              |                      |                      |     "
            "                     |                      |                     "
            "                       |                      |                   "
            "   |\n";
    ...
    for (; !(pbfs.empty() && pdfs.empty() && pdls.empty() && piddfs.empty()
             && pucs.empty() && pastar.empty() && pdijkstra.empty());
         ++i) {
        cout << left << setw(13) << ("    Step " + to_string(i)) << " |"
             << right;
        ...
        if (!piddfs.empty()) {
            ...
            piddfs.pop_front();
            col5 += p.second;
            cout << setw(42 + static_cast<int>(nr_digits_iddfs))
                 << (p.first->first + " (" + to_string(col5) + ")");
        }
        else {
